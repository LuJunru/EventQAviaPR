...
def convert_to_features(data, tokenizer, max_length=300, evaluation=False, output_dir=None, tokenizerfast=None, sep_tok=None):
    ...
    for v_i, v in enumerate(data):
        ...
        # sent_ids: [[0, 10], [10, 17], [17, 40], [40, 109], [109, 180], [-1, -1]]
        # event_ids: [[5, -1, -1], [11, 14, -1], [-1, -1, -1], [42, 89, 91], [-1, -1, -1]]
        # clr_ids: [[2, -1, -1], [1, 0, -1], [-1, -1, -1], [0, 0, 0], [-1, -1,- 1]]
        event_ids = []
        clr_ids = []
        sent_ids = []
        ...
        q_p_sents = sent_tokenize(q_p)
        for q_p_sent in q_p_sents:
            sent_ids.append([])
            outputs_ = tokenizerfast.encode_plus(q_p_sent, add_special_tokens=False, return_offsets_mapping=True)
            sent_l = len(outputs_['offset_mapping'])
            if len(sent_ids) == 1:
                sent_ids[-1].append(0)
            else:
                sent_ids[-1].append(sent_ids[-2][1])
            sent_ids[-1].append(sent_ids[-1][0] + sent_l)
        m_s = max(m_s, len(sent_ids))

        outputs_all = tokenizerfast.encode_plus(q_p, add_special_tokens=False, return_offsets_mapping=True)
        moving = 1 if "prefix" in output_dir else 0
        for loc_i, loc in enumerate(outputs_all['offset_mapping']):
            ...
            if (q_p_t in question_event) or (a_q_p_t in question_event):
                ...
                event_ids.append(loc_i + moving)
                clr_ids.append(0)
        if evaluation is False and sum(clr_ids) < 2:
            logger.info("In some instances, the event triggers are lemmatized by annotators: %s | %s" % (v['question_event'], v['question']))
        
        sent_ids_s = [sent_id[0] for sent_id in sent_ids][1:] + [max_length]  # start index of sent
        new_event_ids = []
        new_clr_ids = []
        s_index = 0
        for event_id_i, event_id in enumerate(event_ids):
            # [10, 14, 39, 43, 55, 59, 72, 76, 80, 135, 158, 171]
            # [66, 97, 110, 141, 159]
            # [[10, 14, 39, 43, 55, 59], [72, 76, 80], [], [135], [158], [171]]
            clr_id = clr_ids[event_id_i]
            while event_id >= s_index:
                s_index = sent_ids_s.pop(0)
                new_event_ids.append([])
                new_clr_ids.append([])
            if event_id < s_index:
                new_event_ids[-1].append(event_id)
                new_clr_ids[-1].append(clr_id)
        for new_event_id_i, new_event_id in enumerate(new_event_ids):
            m_e = max(m_e, len(new_event_id))
            new_event_ids[new_event_id_i] += [-1] * (M_E - len(new_event_ids[new_event_id_i]))
            new_clr_ids[new_event_id_i] += [-1] * (M_E - len(new_clr_ids[new_event_id_i]))
        event_ids = new_event_ids
        clr_ids = new_clr_ids

        if evaluation is False and v_i % 1000 == 0:
            logger.info("=" * 50)
            logger.info("event triggers: %s" % v['original_events']['spans'])
            for sent_id_i, sent_id in enumerate(sent_ids):
                if sent_id[0] == -1:
                    break
                logger.info("-" * 50)
                logger.info("current sent: %s" % (tokenizer.decode(tokenized_ids[sent_id[0]:sent_id[1]])))
                logger.info("extracted triggers: %s" % ([tokenizer.decode(tokenized_ids[event_id]) for event_id in event_ids[sent_id_i] if event_id != -1]))
                logger.info("-" * 50)
            logger.info("=" * 50)
        sent_ids += [[-1, -1]] * (M_S - len(sent_ids))
        event_ids += [[-1] * M_E] * (M_S - len(event_ids))
        clr_ids += [[-1] * M_E] * (M_S - len(clr_ids))
    ...
    logger.info("Maximum number of event triggers per sent is: % s" % (m_e))
    logger.info("Maximum number of sents is: % s" % (m_s))
