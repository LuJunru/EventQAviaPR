...
from utils import *
from models import RobertaSpanPredictor, RobertaSpanPredictor_PR
...
def main():
    ...
    if 'roberta' in args.model:
        ...
        tokenizerfast = RobertaTokenizerFast.from_pretrained(args.model, do_lower_case=args.do_lower_case)
        if "PR" in args.output_dir:
            model = RobertaSpanPredictor_PR.from_pretrained(args.model, mlp_hid=args.mlp_hid_size)
        else:
    ...
    if args.do_train:
        ...
        train_features = convert_to_features(train_data, tokenizer, 
                                             max_length=args.max_seq_length, output_dir=args.output_dir, tokenizerfast=tokenizerfast)
        ...
        logger.info("id_size: {} mask_size: {}, instance_key_size: {}, segment_size: {}, \
        ...
        best_eval_f1 = 0.0
        best_eval_em = 0.0
        if "BIO" in args.output_dir:
            loss_fct = CrossEntropyLoss(weight=torch.tensor([1.0, args.pos_weight, args.pos_weight]).to(device))
        else:
            loss_fct = CrossEntropyLoss(weight=torch.tensor([1.0, args.pos_weight]).to(device))
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                input_ids, input_masks, segment_ids, instance_indices, event_ids, clr_ids, sent_ids, q_start, q_end = batch
                sub_loss = None
                if "PR" in args.output_dir:
                    logits, final_hidden_state, sub_loss = model(input_ids, attention_mask=input_masks, token_type_ids=segment_ids,
                                                                qloc_s=q_start, qloc_e=q_end, sent_i=sent_ids, 
                                                                event_ids=event_ids, clr_ids=clr_ids)
                else:
                    logits, _ = model(input_ids, attention_mask=input_masks, token_type_ids=segment_ids)
            ...
            if args.do_eval:
                ...
                eval_features = convert_to_features(eval_data, tokenizer,
                                                    ...
                                                    output_dir=args.output_dir, tokenizerfast=tokenizerfast)
                ...
                for batch in tqdm(eval_dataloader, desc="Evaluating"):
                    ...
                    with torch.no_grad():
                        if "PR" in args.output_dir:
                            eval_logits, _, _ = model(input_ids, attention_mask=input_masks, token_type_ids=segment_ids, 
                                                      qloc_s=q_start, qloc_e=q_end, 
                                                      sent_i=sent_ids, event_ids=event_ids, clr_ids=clr_ids, eval_type="event")
                        else:
                            eval_logits, _ = model(input_ids, attention_mask=input_masks, token_type_ids=segment_ids)
