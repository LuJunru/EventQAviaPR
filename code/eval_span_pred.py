...
def main():
    ...
    for split in ['dev']:
        ...
        eval_features = convert_to_features(eval_data, tokenizer,
                                            max_length=args.max_seq_length, evaluation=True, output_dir=args.model_dir, tokenizerfast=tokenizerfast)
            ...
            with torch.no_grad():
                if "PR" in args.model_dir:
                    eval_logits, _, _ = model(input_ids, attention_mask=input_masks, token_type_ids=segment_ids, 
                                              qloc_s=q_start, qloc_e=q_end, sent_i=sent_ids, event_ids=event_ids, clr_ids=clr_ids, eval_type="event")
                else:
